...
根据[原博客](https://stopachka.essay.dev/post/5/risp-in-rust-lisp#comparison-operators#language-12-almost-risp)，为了简化代码量，这部分的实现可以用宏实现：
...
同理，def 语法：`(define var exp)`。用于定义一个新的变量 var，它的值是 exp 表达式的值。例如：`(define k1 10)`，逻辑实现如下：
...
我们运行 repl（`cargo r`），通过一些输入，看看实现的效果：
...
当 lambda 被调用时，会生成一个不同于 default_env 的新 env，当执行函数体的时候，会使用新的 env 中的符号、参数等信息，如果查找不到，则在全局环境（default_env）中查找，所以需要调整一下 RispEnv：
...
`data` 是 lambda 内部环境，`outer` 则是外层（全局环境）env。
...
基本完成了 lambda 的支持，我们编译代码试试吧！（`cargo r`）
...
通过使用 Rust 实现 Lisp 子集，我们可以更加熟悉 Rust 的语法和使用，
