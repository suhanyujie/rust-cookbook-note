...
#### 获取 Rc 引用
职能指针 Rc，可以认为是对某个数据的引用，我们可以通过 `Rc::clone()` 的方式复制多份引用，赋值给多个变量，这样可以实现多个变量都指向同一个“树节点”，因为获取引用的调用会比较繁琐，因此我们将其封装为方法 `get_rc()`，放在 impl Tree 块中，其实现如下：
...
### 新增节点
实例化一个只带有根节点的树后，我们还需要将更多的数据加入到树中，因此我们实现 Tree 的 insert 方法。需要注意的是，这里我们还是遵循二叉树的以下性质：二叉树的左节点小于其父节点的值，右子节点值大于根其父节点。insert 实现如下：
...
如果文章有不当之处，或者想和我交流，欢迎[提 issue](https://github.com/suhanyujie/rust-cookbook-note/issues) 和我联系~
